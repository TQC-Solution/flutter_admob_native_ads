...
- **Swift Package Manager (SPM) support** for iOS
  - New `ios/flutter_admob_native_ads/Package.swift` manifest
  - Plugin sources restructured to `ios/flutter_admob_native_ads/Sources/flutter_admob_native_ads/`
  - Works with both CocoaPods and SPM

### Changed
- **Google Mobile Ads SDK upgraded**
  - Android: GMA SDK 25.3.0
  - iOS: GMA SDK 13.4.0 (requires Xcode 26.2+)
- Android `minSdk` raised from 21 to 23 (required by GMA SDK 25.x)
- Kotlin updated to 2.2.20, AGP 8.7.3

### Fixed
- Clear ad cache and callbacks on reload/dispose to prevent stale ads being displayed
- README iOS setup instructions updated to the new `MobileAds.shared` API for GMA SDK 12+ (#18)
